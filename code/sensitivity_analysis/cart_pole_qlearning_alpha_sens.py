...
for alpha_iter in range(len(LEARNING_RATE)):
	...
	for episode in range(EPISODES):
		...
		if not episode % EPISODE_DISPLAY:
			...
			print(f"Episode:{episode} avg:{avg_reward} min:{min(ep_rewards[-EPISODE_DISPLAY:])} max:{max(ep_rewards[-EPISODE_DISPLAY:])} learning rate:{LEARNING_RATE[alpha_iter]}")

env.close()

plt.plot(ep_rewards_table['ep'], ep_rewards_table['zero'], label="0.1")
plt.plot(ep_rewards_table['ep'], ep_rewards_table['one'], label="0.25")
plt.plot(ep_rewards_table['ep'], ep_rewards_table['two'], label="0.75")
plt.plot(ep_rewards_table['ep'], ep_rewards_table['three'], label="1")
plt.legend(loc=4) #bottom right
plt.title('CartPole Q-Learning learning rate sensitivity')
