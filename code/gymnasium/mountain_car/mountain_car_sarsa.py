...
class SARSA:
    ...
    def get_epsilon_greedy_action(self, discrete_state):
        if self.env.default_rng.random() > self.hyper.EPSILON:
            return np.argmax(self.Q_TABLE[discrete_state])
        else:
            return self.env.default_rng.integers(self.env.get_action_space_size())
    ...
    def train(self):
        for episode in tqdm(range(self.hyper.EPISODES)):
            ...
            action = self.get_epsilon_greedy_action(curr_discrete_state)
            ...
            while not terminated:
                ...
                new_action = self.get_epsilon_greedy_action(new_discrete_state)

                if not terminated:
                    current_q = self.Q_TABLE[curr_discrete_state + (action,)]
                    max_future_q = self.Q_TABLE[new_discrete_state + (new_action,)]
...
if __name__ == "__main__":
    logger = setup_logger("mountain_car_sarsa.log")
    logger.info("Mountain Car SARSA")
