...
for eps_iter in range(len(EPSILON)):
	Q_TABLE = np.random.randn(theta_state_size,theta_dot_state_size,env.action_space.n)
	ep_rewards = []
	for episode in range(EPISODES):
		...
		while not done:
			if np.random.random() > EPSILON[eps_iter]:
		...
		if not episode % EPISODE_DISPLAY:
			...
			print(f"Episode:{episode} avg:{avg_reward} min:{min(ep_rewards[-EPISODE_DISPLAY:])} max:{max(ep_rewards[-EPISODE_DISPLAY:])} epsilon:{EPSILON[eps_iter]}")

env.close()

plt.plot(ep_rewards_table['ep'], ep_rewards_table['zero'], label="0.1")
plt.plot(ep_rewards_table['ep'], ep_rewards_table['one'], label="0.25")
plt.plot(ep_rewards_table['ep'], ep_rewards_table['two'], label="0.5")
plt.plot(ep_rewards_table['ep'], ep_rewards_table['three'], label="0.75")
plt.legend(loc=4) #bottom right
plt.title('CartPole Q-Learning epsilon sensitivity')
